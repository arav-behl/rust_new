#!/usr/bin/env python3
"""
Professional Showcase Demo - Real Paper Trading System
Connects to actual trading engine via REST API - NO MOCK DATA
"""
...
with col1:
    ...
    if portfolio:
        ...
        <div class="metric-card">
            <h4>💰 Cash Balance</h4>
            <h2 style="color: #00ff88;">${portfolio.get('cash_balance', 0):,.2f}</h2>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="metric-card">
            ...
            <h2 style="color: #00ff88;">${portfolio.get('total_value', 0):,.2f}</h2>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="metric-card">
            <h4>💹 Total P&L</h4>
            <h2 style="color: {pnl_color};">{pnl_sign}${pnl:.2f}</h2>
        </div>
        """, unsafe_allow_html=True)
...
with col3:
    ...
    if orders:
        ...
        df = pd.DataFrame(orders[-10:])  # Show last 10 orders
        ...
        if not df.empty:
            ...
            display_cols = ['time', 'symbol', 'side', 'quantity', 'price', 'status']
            available_cols = [col for col in display_cols if col in df.columns]

            if available_cols:
                # Style the dataframe
                styled_df = df[available_cols].style.format({
                    'quantity': '{:.4f}',
                    'price': '${:,.2f}' if 'price' in available_cols else '{}'
                })

        ...
        tokio::spawn(async move {
            loop {
                match connect_async(&ticker_url).await {
                    ...
                    Err(e) => {
                        println!("❌ Failed to connect to Binance: {}", e);
        ...
        tokio::spawn(async move {
            loop {
                match connect_async(&depth_url).await {
                    Ok((ws_stream, _)) => {
                        println!("📊 Connected to Binance WebSocket for order book depth");
                        let (mut _write, mut read) = ws_stream.split();
                                ...
                                Ok(Message::Text(text)) => {
                                    if let Ok(depth_data) = serde_json::from_str::<BinanceDepth>(&text) {
                                        ...
                                        if let (Some(best_bid), Some(best_ask)) = (
                                            ...
                                            println!("📖 Order Book Update: {} - Bid: ${:.2} Ask: ${:.2} Spread: ${:.2}",
                                                symbol, bid_price, ask_price, spread);
                                        }
                                    }
                                }
                                Ok(Message::Close(_)) => {
                                    println!("⚠️ Order book WebSocket connection closed");
                                    break;
                                }
                                Err(e) => {
                                    println!("❌ Order book WebSocket error: {}", e);
                                    break;
                                }
                                _ => {}
                            }
                    ...
                    Err(e) => {
                        println!("❌ Failed to connect to Binance order book feed: {}", e);
                        tokio::time::sleep(tokio::time::Duration::from_secs(5)).await;
                    }
                }

                println!("🔄 Reconnecting to order book feed in 5 seconds...");
                tokio::time::sleep(tokio::time::Duration::from_secs(5)).await;
            }
        });
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tokio::spawn(async move {
        ...
        println!("   📖 Live order book depth with bid/ask spreads");
        println!("   ⚡ Realistic order execution at market prices");
        ...
        println!("   • Realistic bid/ask spread execution");
    });
